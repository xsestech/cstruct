//
// This code is my own copy of https://github.com/eteran/c-vector/tree/master
//
...
/**
 * vector_type_t - Type of the vector, that is beeing used in this library
 * @param type type of the vector
 */
#define vector_type_t(type) type *

/**
 * More beautifull to declare vector
 * @param type Type of the vector
 */
 ...
 * @param field Field of vector metdata struct
 ...
 * @param value value, that we will set feild to
 * @internal
 */
...
/**
 * Sets element destuctor in metadata
 * @param vec vector
 ...
 * @param vec vector
 * @returns pointer to data destuctor function
 */
...
/**
 * Initializes vector of size capacity with element destuctor
 * @param vec vector
 * @param capaсicty inital capacity
 * @param elem_destructor_fn pointer to data destructor. Looks like void free_int(void *p) { free(*(int **)p); }
 */
#define vector_init(vec, capaсicty, elem_destructor_fn) \
  do {                                                  \
    if (!(vec)) {                                       \
      vector_reserve((vec), capaсicty);                 \
